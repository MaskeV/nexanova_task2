...
const generateBatchReport = async (req, res) => {
  try {
    ...
    const batch = await Batch.findOne({ batchId }).populate('participants', 'name email');
    if (!batch) {
  ...
  try {
    ...
    for (const evaluation of evaluations) {
      if (!performanceByTechnology[evaluation.technology]) {
        ...
        performanceByTechnology[evaluation.technology] = {
          technology: tech ? { technologyId: tech.technologyId, name: tech.name, rounds: tech.rounds } : null,
          evaluations: [], completedCount: 0, pendingCount: 0, totalScore: 0, averageScore: 0
        };
      }
      performanceByTechnology[evaluation.technology].evaluations.push({
        round: evaluation.round, status: evaluation.status,
        totalScore: evaluation.totalScore, evaluator: evaluation.evaluator,
      ...
      if (!performanceByBatch[evaluation.batch]) {
        ...
        performanceByBatch[evaluation.batch] = {
          batch: batch ? { batchId: batch.batchId, name: batch.name, technology: batch.technology } : null,
          evaluations: [], completedCount: 0, pendingCount: 0, totalScore: 0, averageScore: 0
        };
      }
    ...
    Object.keys(performanceByTechnology).forEach(techId => {
      ...
      perf.averageScore = perf.completedCount > 0
        ? (perf.totalScore / perf.completedCount).toFixed(2) : 0;
    });
    ...
    const completedCount = evaluations.filter(e => e.status === 'completed').length;
    const totalScore = evaluations.filter(e => e.status === 'completed')
      .reduce((sum, e) => sum + (e.totalScore || 0), 0);
    const overallAverage = completedCount > 0 ? (totalScore / completedCount).toFixed(2) : 0;

    res.status(200).json({
      success: true,
      data: {
        participant: { id: participant._id, name: participant.name, email: participant.email },
        overallStatistics: {
          totalEvaluations: evaluations.length,
          completedEvaluations: completedCount,
          ...
          completionRate: evaluations.length > 0
            ? ((completedCount / evaluations.length) * 100).toFixed(2) + '%' : '0%'
        },
    ...
    for (const evaluation of evaluations) {
      if (!batchWorkload[evaluation.batch]) {
        ...
        batchWorkload[evaluation.batch] = {
          batch: batch ? { batchId: batch.batchId, name: batch.name } : null,
          totalAssigned: 0, completed: 0, pending: 0
        };
      }
      batchWorkload[evaluation.batch].totalAssigned++;
      if (evaluation.status === 'completed') batchWorkload[evaluation.batch].completed++;
      else batchWorkload[evaluation.batch].pending++;
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        recentEvaluations: evaluations.slice(0, 10).map(e => ({
          id: e._id, participant: e.participant, batch: e.batch,
          round: e.round, status: e.status,
          assignedDate: e.assignedDate, completedDate: e.completedDate
        }))
      }
    });
  } catch (error) {
...
const getSystemStats = async (req, res) => {
  try {
    ...
    // ✅ FIXED: Count from Participant model, NOT User model
    ...
    res.status(200).json({
      ...
      data: {
        ...
        users: {
          participants: totalParticipants, // ✅ now correct
